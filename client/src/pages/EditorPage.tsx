...
export default function EditorPage() {
  ...
  // ── Collasso gruppi: singolo / tutti ──────────────────────
  // Stato nel padre (come in ViewerPage) per pilotare "Espandi/Comprimi tutti".
  // Un gruppo nuovo non e' nell'insieme -> nasce espanso.
  const [collapsedIds, setCollapsedIds] = useState<Set<string>>(() => new Set())
  const allCollapsed = dettagli.length > 0 && dettagli.every(d => collapsedIds.has(d.id))
  function setCollapsedOne(id: string, next: boolean) {
          ...
          <div className="space-y-3">
            {dettagli.map(det => (
              <DettaglioCard
                ...
                onSetCollapsed={next => setCollapsedOne(det.id, next)}
