...
interface NavItem {
  ...
  /** Intestazione di sezione mostrata SOPRA questa voce (prima di un gruppo). */
  section?: string
}
...
const NAV_ITEMS: NavItem[] = [
  ...
  {
    id:    'verifica-liquidato',
    label: 'Verifica liquidato',
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
  ...
  {
    id:      'emolumenti',
    label:   'Dottorandi e borse',
    section: 'Modulo Emolumenti',
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
...
export default function Layout({ children }: LayoutProps) {
  ...
  const visibleNavItems = NAV_ITEMS.filter(item =>
    ((item.id !== 'utenti' && item.id !== 'audit' && item.id !== 'verifica-liquidato' && item.id !== 'emolumenti') || user?.isAdmin) &&
