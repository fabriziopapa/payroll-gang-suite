...
if [ "$PG_PRONTO" = "1" ]; then
  ...
  if [ "$(psql_q "SELECT 1 FROM pg_database WHERE datname='$DB_NAME'")" = "1" ]; then
    ...
    # Il numero di tabelle era scritto a mano (17) e invecchiava a ogni
    # rilascio che ne aggiungesse una: dal 2026-09-12 sono 19, e il
    # controllo avrebbe segnalato come sbagliato uno schema corretto —
    # un falso allarme permanente, che e' il modo migliore per insegnare
    # a ignorare i controlli.
    #
    # La domanda giusta e' un'altra, e non invecchia: le migrazioni
    # REGISTRATE sono tante quante i file presenti? Se si', lo schema e'
    # quello che il codice si aspetta, qualunque sia il numero di tabelle.
    NT=$(psql_db "SELECT count(*) FROM information_schema.tables WHERE table_schema='public'")
    NREG=$(psql_db "SELECT count(*) FROM schema_migrations")
    NFILE=$(find "$APP_DIR/server/src/db/migrations" -maxdepth 1 -name '[0-9]*.sql' 2>/dev/null | wc -l)
    if [ -z "$NT" ] || [ "$NT" = "0" ]; then
      todo "Schema vuoto — eseguire server/sql/setup.sql"
    elif [ -z "$NREG" ]; then
      avv "Schema: $NT tabelle, ma il registro 'schema_migrations' non esiste — eseguire ./pgs-migra.sh baseline"
    elif [ "$NREG" = "$NFILE" ]; then
      ok "Schema: $NT tabelle · $NREG migrazioni registrate su $NFILE file (allineato)"
    else
      avv "Schema: $NREG migrazioni registrate su $NFILE file presenti — eseguire ./pgs-migra.sh applica"
    fi
