...
| Keep live supabase (`<IP_VPS>/supabase_keepalive.php`) | ⚠️ altro progetto | appartiene al secondo sito PHP sulla VPS — fuori scope Payroll, decidere a parte |
...
|---|---|---|
| `/www/wwwroot/<IP_VPS>/` (1.2 MB) | `supabase_keepalive.php` (tiene vivo Supabase free-tier, notifiche Telegram) + script upload Cubbit | **SÌ** — copia diretta |
| `/www/keepalive_private/` | config privata del keepalive (contiene segreti — mai committare) | **SÌ** — copia diretta, permessi `640 www:www` |
...
| Vhost nginx `<IP_VPS>` | sito **fermato** (root → `/www/server/stop/`) — serve solo il cron CLI | NO (ricreare solo se riattivi gli upload web) |
...
#      scp -r /www/wwwroot/<IP_VPS> /www/keepalive_private nuova:/tmp/...
...
ls /mnt/cubbit-media/<IP_VPS>/    # devono comparire i file del bucket
...
30 3 */4 * * root /usr/bin/php /www/wwwroot/<IP_VPS>/supabase_keepalive.php >> /www/keepalive_private/cron.log 2>&1
EOF

# ── 8. Test immediato + verifica notifica Telegram
php /www/wwwroot/<IP_VPS>/supabase_keepalive.php; tail -5 /www/keepalive_private/keepalive_log.txt
```
