...
// ------------------------------------------------------------
// EMOLUMENTI — LAVORAZIONI (area dottorandi e borsisti)
// Tabella SEPARATA da `bozze`: l'area Liquidazioni non si tocca (§8.1).
// `dati` conserva l'input dell'operatore E lo snapshot di cio' che CSA
// mostrava al salvataggio, cosi' riaprendo si vede lo stesso quadro senza
// rileggere CSA — e si puo' confrontare con la lettura fresca.
// ------------------------------------------------------------

export const emolumentiLavorazioni = pgTable('emolumenti_lavorazioni', {
  id:               uuid('id').primaryKey().defaultRandom(),
  nome:             varchar('nome', { length: 200 }).notNull(),
  /** 'bozza' = in lavorazione · 'archiviata' = chiusa */
  stato:            varchar('stato', { length: 20 }).notNull().default('bozza'),
  /** DR | BS | BE — facoltativo, serve solo a ritrovarle */
  tipo:             varchar('tipo', { length: 10 }),
  /** Richiesta all'archiviazione, come per le bozze di liquidazione */
  dataLiquidazione: date('data_liquidazione'),
  /** ID liquidazione CSA, es. "1ND090005001220240442801". Facoltativo,
   *  come nelle bozze: spesso arriva dopo l'archiviazione. */
  idLiquidazioneCsa: varchar('id_liquidazione_csa', { length: 40 }),
...
export type EmolumentiLavorazione    = typeof emolumentiLavorazioni.$inferSelect
export type NewEmolumentiLavorazione = typeof emolumentiLavorazioni.$inferInsert
