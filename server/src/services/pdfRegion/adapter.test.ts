...
test('adaptToParsed — campi anagrafica 1:1 (CF, nascita, inquadramento, sede, …) mappati direttamente', () => {
  ...
  const r = adaptToParsed(extraction([
    t('a1', 'anagrafica', '000456'),
    t('a2', 'anagrafica', 'VRDLGU85M15H501X'),
    t('a3', 'anagrafica', '15/08/1985'),
    t('a4', 'anagrafica', 'ROMA (RM)'),
    t('a5', 'anagrafica', 'Area degli Operatori'),
    t('a6', 'anagrafica', 'Settore dei servizi generali e tecnici'),
    t('a7', 'anagrafica', 'ND'),
    t('a8', 'anagrafica', '01/03/2024'),
    t('a9', 'anagrafica', 'anni 01 mesi 00 giorni 26'),
    t('a10', 'anagrafica', 'Ufficio di Esempio'),
    t('a11', 'anagrafica', 'Sede Centrale'),
    t('v1', 'descrizione', 'Retribuzione complessiva'), t('v1', 'importo', '1.234,56'),
  ]), parti)

  const a = r.parsed.anagrafica
  assert.equal(a.codice_fiscale,     'VRDLGU85M15H501X')
  assert.equal(a.data_nascita,       '15/08/1985')
  assert.equal(a.luogo_nascita,      'ROMA (RM)')
  ...
  assert.equal(a.inizio_rapporto,    '01/03/2024')
  assert.equal(a.anzianita_servizio, 'anni 01 mesi 00 giorni 26')
  assert.equal(a.afferenza,          'Ufficio di Esempio')
  assert.equal(a.sede,               'Sede Centrale')
  assert.equal(r.errors.length, 0)
})
