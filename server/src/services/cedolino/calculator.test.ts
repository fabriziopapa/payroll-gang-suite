...
// ── Campione Esposito Enzo, Mag 2026 (ND con Abb.TFR) ──────────────────────
// Verifica regola Excel: Abb.TFR DPCM 20.12.99/L.335/95 inglobato nelle
// ritenute previdenziali del certificato (anomalia certificato 2026/002).
...
test('campione Esposito — Abb.TFR inglobato nelle previdenziali (Excel ufficio)', () => {
...
test('campione Esposito — riassunto verifica: gruppi e inglobamenti', async () => {
...
test('senza abbattimenti il calcolo resta invariato (campione Pino)', () => {
  // Pino non ha Abb.TFR: la nuova sezione non deve alterare i valori storici.
  const c = computeCertificato(TEORICHE, VOCI, RIEPILOGO)
  assert.equal(c.ritenute_previdenziali, 247.68)
})
