...
│       │   ├── migrations/      # 0001…0019 — applicate da ./pgs-migra.sh, registrate in schema_migrations
...
- Nuova tabella `paesi_conto`, con l'elenco iniziale EPC v8.0 (42 prefissi). Una riga in vigore per paese: un paese non puo' essere insieme SEPA ed EXTRA_UE. I cambi non sovrascrivono: la storia resta, e l'endpoint accetta una data.
- Card **Paesi e area del conto** in *Impostazioni*: tutti la vedono, un amministratore cambia l'area di un paese da una data con un motivo. Nota "in fase di sviluppo" e link alla fonte primaria EPC.
- `GET /api/v1/area-conto` accetta `data=AAAA-MM-GG` e dice se l'elenco viene dal database o dal codice.
- Nessun aggiornamento automatico: una fonte affidabile leggibile dagli script non e' stata trovata (EPC dietro Cloudflare; elenco UE incompleto).
...
*Nessuna migrazione. Liquidazioni non toccate.*

- **Nuovo pulsante nella lavorazione**: legge le testate di una liquidazione CSA (anno, mese, ruoli, progressivo facoltativo) e dice per ogni riga su quale conto CSA ha pagato. Le differenze con la riga si confermano una per una; confermati, da chiarire e non in liquidazione restano segnati sulla riga, anche dopo la cancellazione della liquidazione in CSA.
- **Privacy**: della testata sopravvivono matricola, progressivo e nazione. La normalizzazione (`services/emolumenti/testate.ts`) e' logica pura con i suoi test, incluso quello che verifica che IBAN e intestazione non passino.
- *Aggiorna ruoli e conti* non sovrascrive piu' da solo un conto **confermato da CSA** con la stima dell'anagrafica: lo chiede.
- Corretto il testo del pannello dettagli: una riga senza area non vuol dire "persona non trovata".
...
*Nessuna modifica all'applicazione. Tre incidenti in quattro giorni avevano tutti la stessa causa — operazioni sullo schema e sui permessi di cui nessuno verificava l'esito — e questa versione chiude quella causa, non i sintomi.*

- **`schema_migrations` e `pgs-migra.sh`.** Fino a oggi le migrazioni si applicavano incollando `psql` in SSH, e da nessuna parte era scritto **quali** fossero state applicate: lo sapeva solo chi le aveva lanciate, su due macchine diverse. `npm run migrate` (`drizzle-kit migrate`) non e' mai stato eseguito in produzione — `drizzle.config.ts` non esiste e la tabella `__drizzle_migrations` non c'era. Ora ogni migrazione e' registrata con il suo **checksum**, si applica **una volta sola** e nella **stessa transazione** della propria registrazione: lo stato "applicata ma non registrata" non esiste piu'. Un file modificato dopo essere stato applicato **ferma tutto** invece di far divergere in silenzio due macchine — una migrazione applicata e' storia, non codice.
- **Il deploy ora applica le migrazioni, e se falliscono non prosegue** (`pgs-update-aapanel.sh`, nuovo passo **[3/7]**). Il 2026-09-12 la 0015 fu rifiutata con *must be owner of table bozze* e il deploy ando' avanti comunque: codice nuovo contro schema vecchio, Dashboard rotta. La causa non era la migrazione, era che il suo esito non veniva guardato. Se il passo [3/7] esce diverso da zero non si compila e non si riavvia: l'app resta alla versione precedente, che funziona.
- **Un solo proprietario: il superutente.** `payroll_user` non possiede piu' alcun oggetto, e non e' una formalita' — senza la proprieta' non puo' fare `ALTER` ne' `DROP` sulle tabelle esistenti (verificato: `DROP TABLE bozze` respinto con *must be owner of table*), quindi un difetto nel codice server puo' al massimo danneggiare i dati, non portarsi via la struttura. Il blocco `DO` di `setup.sql` che passava tutto a `payroll_user` e' stato **rimosso**, e `owner_payroll_user.sql` e' sostituito da **`proprieta_postgres.sql`**, che fa l'opposto ed e' quello che serve dopo un ripristino dal pannello aaPanel — il restore gira come `payroll_user` e annullerebbe la regola da solo.
- **`permessi.sql`, con autoverifica.** I privilegi non sono un atto d'installazione: sono un invariante da riaffermare dopo ogni modifica dello schema. Il file concede, revoca **in quest'ordine obbligato** — il generale sovrascrive il particolare — e poi **controlla**, sollevando un'eccezione se lo stato non e' quello atteso. Due trappole ora sono scritte nel file e non nella memoria di qualcuno: un `GRANT` dato *prima* di un `ALTER ... OWNER TO` viene **riassorbito** (cambiare proprietario riscrive la ACL e rimappa le voci del vecchio proprietario sul nuovo — e' cosi' che Emolumenti e' andato in 500), e `ALTER DEFAULT PRIVILEGES` **riconcede** `UPDATE`/`DELETE` su ogni tabella creata dopo.
- **L'immutabilita' di `audit_log` non era in vigore in produzione**, pur essendoci il `REVOKE` in `setup.sql`. Causa dimostrata in laboratorio: basta che la tabella venga **ricreata** una volta e le *default privileges* le ridanno `UPDATE` e `DELETE`, in silenzio. Non serve che nessuno sbagli. Ora `permessi.sql` la riporta append-only e **fallisce** se non ci riesce, e il runner lo rilancia dopo ogni migrazione.
- **`pgs-installa.sh`: primo avvio in un comando.** Controlla che `.env` contenga `ENCRYPTION_KEY` e le chiavi JWT — vuote non impediscono l'avvio, rompono la prima lettura di un dato cifrato, che e' molto peggio — crea ruolo, database, schema e permessi, marca le migrazioni gia' contenute nel consolidato, e verifica il risultato con una **connessione reale usando le credenziali dell'applicazione**, non quelle del superutente: e' li' che si scoprono host sbagliato e `pg_hba` che non ammette `scram`. Chiude dicendo cio' che non puo' fare al posto tuo: il primo utente.
- **`CREATE ON SCHEMA public` resta a `payroll_user`**, e la riga in `setup.sql` era giusta — solo giustificata con la ragione sbagliata. Serve ai **ripristini** (il pannello aaPanel e `cpanel-restore-dump.sh` girano come `payroll_user`), non al DDL sulle tabelle esistenti, che dipende dalla proprieta'. Toglierla e' comunque impossibile senza spostare lo schema: da PostgreSQL 15 `public` appartiene a `pg_database_owner`, e il database e' di `payroll_user`. `permessi.sql` lo segnala a video invece di fingere che sia un problema.
- **Hook `commit-msg`**: blocca le righe di attribuzione automatica nei messaggi di commit. Su un repository pubblico con storia permanente, togliere una riga entrata per distrazione richiede un force-push, e un force-push manda il deploy in recupero con bundle e reset. La regola era affidata alla memoria: ora e' una proprieta' del repository.
- **Provato, non dedotto.** L'intera procedura e' stata eseguita su un PostgreSQL vuoto: installazione da zero (exit 0, 18 tabelle piu' il registro, proprieta' del superutente, `audit_log` immodificabile, connessione applicativa riuscita) e poi i casi di guasto uno per uno — migrazione che fallisce a meta' (nulla registrato, DDL parziale annullato), file alterato a posteriori (bloccato con i due checksum a video), deriva di proprieta' (rilevata e riparata), `audit_log` ricreata (rilevata e riportata immutabile).
