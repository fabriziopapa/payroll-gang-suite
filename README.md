...
- **IP client da `CF-Connecting-IP` (SEC-A5) — la correzione risolutiva dietro Cloudflare.** Il solo `trustProxy` loopback non basta con la CDN: la catena `X-Forwarded-For` resta pilotabile dal client (un hop loopback in coda fa restituire a `proxy-addr` il valore falso). L'app ora ricava l'IP per audit e rate limiting da `CF-Connecting-IP` (helper `server/src/lib/clientIp.ts`, con fallback a `request.ip` per ambienti senza CDN) e usa lo stesso valore come `keyGenerator` dei due rate limiter. Verificato su pre-prod e produzione: gli `X-Forwarded-For` falsi vengono ignorati, `audit_log` registra l'IP reale.
- **Chiusura dell'origine a Cloudflare (`pgs-cf-origin-lock.sh`)**: `CF-Connecting-IP` e' affidabile solo se l'origine accetta 80/443 dai soli range Cloudflare (altrimenti si colpisce l'IP diretto e si imposta l'header). Lo script rileva il firewall (CSF/ufw/firewalld, con fallback nftables), non tocca SSH, ed e' rieseguibile. *(Applicazione firewall in corso di verifica per macchina.)*
...
- **Rilascio come script**: `pgs-push.ps1` (push da Windows, LF), deploy `pgs-update.sh` (cPanel/systemd) e `pgs-update-aapanel.sh` (aaPanel/PM2 come utente `www`) — entrambi gestiscono in automatico la divergenza git da force-push (backup + `reset --hard`); collaudo `pgs-xff-check.sh`. Vedi *Deploy → Rilascio di sicurezza*.
