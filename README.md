...
- **Entrando nel campo si seleziona tutto**, con il mouse come con Tab: la prima cifra digitata sostituisce quello che c'era. Serviva un doppio click per selezionare, e portare semplicemente il cursore a zero non basterebbe — con `03/2005` gia' scritto, digitare `072026` da li' darebbe `07202603/2005`, dodici cifre, dritto nel ramo d'errore. L'`onMouseUp` che segue il focus va intercettato una volta sola, altrimenti il browser al rilascio del mouse collassa la selezione sul punto cliccato; un secondo click, a campo gia' attivo, resta libero di posizionare il cursore.
