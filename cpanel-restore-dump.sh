...
# PERCHE' QUESTO BLOCCO E' COSI' LUNGO. Prima era tre righe, e mentiva.
# `postgres` non puo' scrivere in /root (permessi 700), quindi il pg_dump
# diretto la' dentro FALLISCE SEMPRE. Il fallimento veniva attribuito a
# "database vuoto o appena creato", declassato ad avviso, e il ripristino
# proseguiva senza rete — poi in coda lo script annunciava il backup a un
# percorso dove non c'era niente. Il 2026-09-12 ha sovrascritto 4147
# anagrafiche e 42 liquidazioni dichiarando che era tutto a posto.
#
# Un errore che mente in modo rassicurante e' peggio di un errore che
# grida: il secondo lo vedi, il primo lo scopri quando ti serve il backup.
#
# Ora: si scrive dove postgres puo' (come fa la fase 4 per leggere il
# dump), l'errore vero si stampa, e se il database CONTIENE DATI il
# fallimento e' fatale. Si prosegue senza backup solo con una scelta
# esplicita, PGS_SENZA_BACKUP=1.
BK="/root/pgs_pre_restore_$(date +%Y%m%d_%H%M%S).dump"
STAGE_BK=$(mktemp -d /tmp/pgs-backup-XXXXXX) || die "Impossibile creare la directory temporanea del backup."
chown postgres:postgres "$STAGE_BK"; chmod 700 "$STAGE_BK"

if sudo -n -u postgres pg_dump -Fc --no-owner --no-privileges -d "$DB_NAME" \
     -f "$STAGE_BK/pre-restore.pgc" </dev/null 2>"$STAGE_BK/errore.log"; then
  mv "$STAGE_BK/pre-restore.pgc" "$BK"
  chmod 600 "$BK"
  rm -rf "$STAGE_BK"
  # Un pg_dump che esce 0 lasciando un file vuoto e' la stessa bugia di
  # prima con un'altra faccia: il backup "c'e'" e non contiene niente.
  DIM_BK=$(stat -c %s "$BK" 2>/dev/null || echo 0)
  if [ "$DIM_BK" -lt 1024 ]; then
    die "Il backup e' stato creato ma pesa $DIM_BK byte: non e' un backup.
       Non proseguo. File: $BK"
  fi
  ok "Backup salvato: $BK ($(du -h "$BK" | cut -f1))"
else
  echo
  sed -n '1,5p' "$STAGE_BK/errore.log" 2>/dev/null | sed 's/^/      /'
  echo
  rm -rf "$STAGE_BK"

  # "Vuoto" va deciso sui conteggi letti nella fase 1, non sull'esito del
  # pg_dump: sono due domande diverse, e confonderle e' stato il difetto.
  VUOTO=1
  for N in "$PRIMA_U" "$PRIMA_A" "$PRIMA_B"; do
    case "$N" in ''|0) : ;; *) VUOTO=0 ;; esac
  done

  if [ "$VUOTO" = "1" ]; then
    avv "Backup non eseguito: il database e' vuoto, non c'era nulla da salvare."
    BK="(nessuno: database vuoto)"
  elif [ "${PGS_SENZA_BACKUP:-0}" = "1" ]; then
    avv "Backup FALLITO, ma PGS_SENZA_BACKUP=1: proseguo su tua scelta esplicita."
    BK="(nessuno: backup fallito, proseguito con PGS_SENZA_BACKUP=1)"
  else
    die "BACKUP FALLITO e il database CONTIENE DATI (utenti=$PRIMA_U anagrafiche=$PRIMA_A liquidazioni=$PRIMA_B).
       Non proseguo: il ripristino sovrascriverebbe senza rete. Sopra l'errore vero di pg_dump.
       Se vuoi procedere comunque, sapendo che perdi il contenuto attuale:
         PGS_SENZA_BACKUP=1 bash $0 $DUMP"
  fi
fi
...
# grep -c stampa "0" ED esce con codice 1 quando non trova nulla: il
# vecchio `|| echo 0` accodava un secondo zero e la variabile diventava
# "0\n0", cosi' il confronto numerico sotto falliva sempre con
# "integer expression expected". Il conteggio degli errori di
# pg_restore era una verifica che c'era solo all'apparenza.
NERR=$(grep -c "^pg_restore: error" /tmp/pgs_restore.log 2>/dev/null || true)
[ -n "$NERR" ] || NERR=0
